...
function deleta(string $entidade,array $criterio = [1]): bool
{
    $retorno = false;
    $coringa_criterio = [];
    foreach ($criterio as $expressao){
        ...
        $expressao[count($expressao)-1]='?';
        $coringa_criterio[] = $expressao;
        $nome_campo = (count($expressao)<4) ? $expressao[0] : $expressap[1];
        $compos_criterio[] = $nome_campo;
        $$nome_campo = $dado;
    }
...
string $ordem = null) : array
{
    ...
    $instricao = select($entidade, $campos,$coringa_criterio, $ordem);
    $conexao = conecta ();
    $stmt = mysqli_prepare($conexao, $instrucao);
